...
python perform_packagecopy () {
    ...
    # PATCHED: Use temp file instead of pipe to avoid Dunfell pseudo
    # *at() syscall crash on pipe fds (Ubuntu 22.04 / kernel 6.x)
    import tempfile
    tmpf = tempfile.mktemp(prefix='pkgcopy-', suffix='.tar', dir=d.getVar('WORKDIR'))
    cmd = 'tar -cf %s -C %s -p -S --warning=no-file-changed --warning=no-file-removed .' % (tmpf, dest)
    subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT)
    cmd = 'tar -xf %s -C %s' % (tmpf, dvar)
    subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT)
    try:
        os.unlink(tmpf)
    except OSError:
